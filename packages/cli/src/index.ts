...
// ---------------------------------------------------------------------------
// strus kc — Knowledge Component management
// ---------------------------------------------------------------------------

const kcCmd = program.command("kc").description("Knowledge Component commands");

/**
 * strus kc seed
 * Seeds all structural KCs (case, number, gender, tense, mood, pos).
 * Idempotent — safe to run multiple times.
 */
kcCmd
  .command("seed")
  .description("Seed structural KCs (case, number, gender, tense, mood, pos). Idempotent.")
  .action(async () => {
    const dbPath = process.env.STRUS_DB_PATH;
    if (!dbPath) {
      console.error(
        "Error: STRUS_DB_PATH environment variable is required for this command.\n" +
        "Usage: STRUS_DB_PATH=/path/to/strus.db strus kc seed",
      );
      process.exit(1);
    }
    ...
    const SEEDS = [
      ...
      { id: "kc-pos-verb",       kind: "pos", label: "verb",       labelPl: "czasownik",  tagPattern: "fin,praet,bedzie,impt,cond,imps,inf,pcon,pant,pact,ppas,ger,winien" },
      ...
      { id: "kc-pos-participle", kind: "pos", label: "participle", labelPl: "imiesłów",   tagPattern: "pcon,pant,pact,ppas" },
    ] as const;
    ...
    for (const seed of SEEDS) {
      ...
      console.log(`  + [${seed.kind}] ${seed.label}`);
      created++;
    }
...
kcCmd
  ...
  .description(
    ...
    "Also creates lemma-kind KCs for each lemma that has morph_form cards. " +
  ...
  .action(async () => {
    ...
    if (!dbPath) {
      console.error(
        "Error: STRUS_DB_PATH environment variable is required for this command.\n" +
        "Usage: STRUS_DB_PATH=/path/to/strus.db strus kc backfill",
      );
      process.exit(1);
    }
    ...
    if (structuralKCs.length === 0) {
      console.warn("No structural KCs found — run 'strus kc seed' first.");
      process.exit(1);
    }
    ...
    const morphCards = db
      .select({
        ...
        lemmaId: notesTable.lemmaId,
      })
      .from(cardsTable)
      .innerJoin(notesTable, eq(cardsTable.noteId, notesTable.id))
      .where(eq(cardsTable.kind, "morph_form"))
      .all();
    ...
    for (const card of morphCards) {
      ...
      if (card.lemmaId) {
        ...
        if (!lemmaKC) {
          ...
          lemmaKC = { id: kcId, kind: "lemma", label: `lemma:${card.lemmaId}`, labelPl: null, tagPattern: null, lemmaId: card.lemmaId, createdAt: now };
    ...
    for (let i = 0; i < pairsToInsert.length; i += BATCH_SIZE) {
      insertBatch(pairsToInsert.slice(i, i + BATCH_SIZE));
    }

    const totalJunction = sqlite
      .prepare("SELECT COUNT(*) as c FROM card_knowledge_components")
      .get() as { c: number };

    console.log(
      `\nBackfill complete.` +
      `\n  Processed: ${morphCards.length} morph_form cards` +
      `\n  Pairs inserted (or ignored): ${pairsToInsert.length}` +
      `\n  Lemma KCs created: ${lemmaKCsCreated}` +
      `\n  Total card↔KC rows in DB: ${totalJunction.c}`,
    );
  });
