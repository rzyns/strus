...
/**
 * strus kc seed
 * Seeds all structural KCs via the API.
 * Idempotent — safe to run multiple times.
 */
kcCmd
  .command("seed")
  .description("Seed structural KCs (case, number, gender, tense, mood, pos) via the API. Idempotent.")
  .action(async () => {
    const result = await apiPost<{ created: number; skipped: number }>("/api/kc/seed", {});
    console.log(`Done. Created: ${result.created}, already existed: ${result.skipped}.`);
  });

/**
 * strus kc backfill
 * Populates card_knowledge_components for all existing morph_form cards via the API.
 */
kcCmd
  .command("backfill")
  .description(
    "Populate card_knowledge_components for all existing morph_form cards via the API. " +
    "Also creates lemma-kind KCs for each lemma that has morph_form cards. " +
    "Idempotent — uses INSERT OR IGNORE.",
  )
  .action(async () => {
    console.log("Running backfill via API (this may take a moment with many cards)...");
    const result = await apiPost<{
